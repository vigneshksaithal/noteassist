...
function installBun(platform, dst) {
  ...
  try {
    ...
    let { exitCode } = spawn(
      "npm",
      ["install", "--loglevel=error", "--prefer-offline", "--no-audit", "--progress=false", `${module2}@1.1.27`],
      {
        cwd,
        stdio: "pipe",
...
function downloadBun(platform, dst) {
  return __async(this, null, function* () {
    let tgz = yield (yield fetch(`https://registry.npmjs.org/@oven/${platform.bin}/-/${platform.bin}-1.1.27.tgz`)).arrayBuffer(), buffer;
    try {
